...
### `jobclaw login-llm` — 检查 LLM 认证状态

```bash
jobclaw login-llm --check
```
...
### `jobclaw init-profile` — AI 引导生成求职画像
...
回答 13 个问题（名字、技能、期望薪资等），AI 自动生成标准 YAML 画像文件。内置校验和纠错机制，确保生成内容合法。
